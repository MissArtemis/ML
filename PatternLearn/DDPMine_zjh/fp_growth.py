...
def find_frequent_itemsets(transactions, minimum_support, include_support=False):
    ...
    def clean_transaction(transaction):
        ...
        transaction.itemset = list(filter(lambda v: v in items, transaction))
        # # print(transaction)
        #
        transaction.itemset.sort(key=lambda v: items[v], reverse=True)
        return transaction
...
if __name__ == '__main__':
    ...
    f = open("./adult2.csv")
    try:
        for itemset in find_frequent_itemsets(csv.reader(f), 1):
            print ('{' + ', '.join(itemset) + '}')
    finally:
        f.close()

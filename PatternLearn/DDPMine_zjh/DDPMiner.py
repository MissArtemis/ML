...
class DDPMine:
 ...
 #########################################################################################################################################
            lb = self._globalTransactionDatabase.label(size)
            self._label = lb
            start = time.clock()
            ...
            print("Transactions: %d – found best pattern '%s' to %s in %f seconds" % (size, "".join([] if not self._bestPattern else self._bestPattern), lb,elapsed))
            ...
            self._bestPatterns.append([self._bestPattern,self._label])
    ...
    def branchAndBound(self,tree,minimum_support,suffix):
        ...
        for item, nodes in tree.items():
            ...
            if support >= minimum_support and item not in suffix:
                ...
                #print support
                #print "---"
                # # print("self._globalTransactionDatabase.size() "+self._globalTransactionDatabase.size())
                # print("self._globalTransactionDatabase.labelSupport() "+self._globalTransactionDatabase.labelSupport())
                # print("self._globalTransactionDatabase.labelAndPatternSupport(found_set) "+self._globalTransactionDatabase.labelAndPatternSupport(found_set))
                ...
                print("qqq",conditionalDatabase.size()/self._globalTransactionDatabase.size(),self._globalTransactionDatabase.labelSupport())
                ...
                print("infoGainBound:",infoGainBound)

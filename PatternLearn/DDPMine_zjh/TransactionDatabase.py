...
class TransactionDatabase(object):
    ...
    #############################################
    def label(self,size):
        return self.transactions[size-1].label
    ...
    def labelSupport(self):
        ...
        if self.dbChangedBool :
            ...
            for transaction in self.transactions :
                ...
                for lb in self.labelSupportiveSymbol:
                    if(transaction.label == lb):
                        count += 1
    ...
    def labelAndPatternSupport(self, pattern):
        ...
        if patternSet == self.lastPatternChecked :
            ...
            for transaction in self.transactions :
                for lb in self.labelSupportiveSymbol:
                    if(transaction.label == lb and patternSet.issubset(transaction.frozitemset)):
                        count += 1
    ...
    def removeTransactions(self, transaction_ids):
        ...
        """
        self.transactions = list(filter(lambda t: t.id not in transaction_ids, self.transactions))
    ...
    def cleanAndPrune(self, min_support):
        ...
        def clean_transaction(transaction):
            transaction.itemset = list(filter(lambda v: v in self.itemSupportDict, transaction.itemset))
